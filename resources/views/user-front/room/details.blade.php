                                    ...
                                    <div class="main-slider">
                                        @foreach ($sliderImages as $image)
                                            <div class="single-img">
                                                <a href="{{ asset('assets/img/rooms/slider-images/' . $image) }}"
                                                    class="main-img">
                                                    <img src="{{ asset('assets/img/rooms/slider-images/' . $image) }}"
                                                        alt="Image">
                                                </a>
                                            </div>
                                        @endforeach
                            ...
                            <div class="d-flex align-items-center justify-content-between mb-4">
                                @if ($roomSetting->room_category_status == 1)
                                    <div class="room-cat mb-0">
                                        <a
                                            href="{{ route('front.user.rooms', ['username' => getParam(), 'category' => $details->roomCategory->id]) }}">{{ $details->roomCategory->name }}</a>
                                    </div>
                                @endif
                                        ...
                                        <div role="tabpanel" class="tab-pane fade" id="reviews">
                                            ...
                                            @guest('customer')
                                                <h5>{{ $keywords['Please'] ?? 'Please' }}
                                                    <a class="text-primary"
                                                        href="{{ route('customer.login', ['username' => getParam(), 'redirectPath' => 'room_details']) }}">{{ $keywords['Login'] ?? 'Login' }}</a>
                                            ...
                                            @auth('customer')
                                                <div class="review-form">
                                                    ...
                                                    <form
                                                        action="{{ route('front.user.room.store_review', ['username' => getParam(), 'id' => $details->room_id]) }}"
                                                        method="POST">
                                                        @csrf
                                                        <div class="mb-25">
                            ...
                            @else --}}
                                <form action="{{ route('front.user.room_booking', getParam()) }}" method="POST"
                                    ...
                                    <div class="mb-2">
                                        <div class="input-wrap">
                                            <input type="text"
                                                placeholder="{{ $keywords['Number_of_Nights'] ?? 'Number of Nights' }}"
                                                id="night" name="nights" value="{{ old('nights') }}" readonly>
                                            <i class="fas fa-moon"></i>
                                        </div>
                                        <small
                                            class="text-primary mt-2 {{ $currentLanguageInfo->direction == 0 ? 'ml-2' : 'mr-2' }} mb-0">
                                    ...
                                    <div class="mb-2">
                                        <div class="input-wrap">
                                            <input type="text"
                                                placeholder="{{ $keywords['Number_of_Guests'] ?? 'Number of Guests' }}"
                                    ...
                                    <div class="mb-2">
                                        <div class="input-wrap">
                                            @guest('customer')
                                                <input type="text"
                                                    placeholder="{{ $keywords['Full_Name'] ?? 'Full Name' }}"
                                                    name="customer_name" value="{{ old('customer_name') }}">
                                            @endguest

                                            @auth('customer')
                                                @php
                                                    if (!empty(Auth::guard('customer')->user()->first_name) || !empty(Auth::guard('customer')->user()->last_name)) {
                                    ...
                                    <div class="mb-2">
                                        <div class="input-wrap">
                                            @guest('customer')
                                                <input type="text" placeholder="{{ $keywords['phone'] ?? 'Phone' }}"
                                    ...
                                    <div class="mb-2">
                                        <div class="input-wrap">
                                            @guest('customer')
                                                <input type="email" placeholder="{{ $keywords['email'] ?? 'Email' }}"
                                    ...
                                    <div class="row {{ old('paymentType') == 'stripe' ? '' : 'd-none' }} "
                                        id="tab-stripe">
                                        <div class="col-12">
                                            <div class="field-input">
                                                <input type="text" class="card-elements mb-2" name="cardNumber"
                                                    placeholder="{{ $keywords['Card_Number'] ?? 'Card Number' }}"
                                                    autocomplete="off" value="{{ old('cardNumber') }}">
                                            </div>
                                            @error('cardNumber')
                                        ...
                                        <div class="col-12">
                                            <div class="field-input">
                                                <input type="text" class="card-elements mb-2"
                                                    placeholder="{{ $keywords['CVC_Number'] ?? 'CVC Number' }}"
                                                    name="cardCVC" value="{{ old('cardCVC') }}">
                                            </div>
                                            @error('cardCVC')
                                                <p class="ml-2 mb-2 text-danger">{{ convertUtf8($message) }}</p>
                                            @enderror
                                        ...
                                        <div class="col-12">
                                            <div class="field-input">
                                                <input type="text" class="card-elements mb-2"
                                                    placeholder="{{ $keywords['Expiry_Month'] ?? 'Expiry Month' }}"
                                        ...
                                        <div class="col-12 mb-4">
                                            <div class="field-input">
                                                <input type="text" class="card-elements"
                                                    placeholder="{{ $keywords['Expiry_Year'] ?? 'Expiry Year' }}"
                                    ...
                                    <div style="{{ old('paymentType') == 'authorize.net' ? '' : 'd-none' }}"
                                        id="authorizenet-form">
                                        <div class="row  ">
                                            <div class="col-md-12 mb-4">
                                                <div class="form_group">

                                                    <input type="text" class="form_control" id="AcardNumber"
                                                        autocomplete="off" name="AuthorizeCardNumber"
                                            ...
                                            <div class="col-md-12 mb-4">
                                                <div class="form_group">
                                                    ...
                                                    <input type="text" class="form_control" id="AcardCode"
                                                        autocomplete="off" name="AuthorizeCardCode"
                                            ...
                                            <div class="col-md-12 mb-4">
                                                <div class="form_group">
                                                    ...
                                                    <input type="text" class="form_control" id="AexpMonth"
                                                        name="AuthorizeMonth"
                                            ...
                                            <div class="col-md-12 mb-4">
                                                <div class="form_group">
                                                    ...
                                                    <input type="text" class="form_control" id="AexpYear"
                                                        name="AuthorizeYear"
                                    ...
                                    <div class="price-option-table mt-4">
                                        <ul>
                                            ...
                                            <li class="single-price-option">
                                                <span class="title">{{ $keywords['Discount'] ?? 'Discount' }} <span
                                                        class="text-success">(<i class="fas fa-minus"></i>)</span> <span
                                                        class="amount">{{ $position == 'left' ? $symbol : '' }}<span
                        ...
                        <div class="single-room">
                            <a class="room-thumb d-block"
                                ...
                                <div class="room-price">
                                    <p>{{ $position == 'left' ? $symbol : '' }}{{ $latestRoom->room->rent }}{{ $position == 'right' ? $symbol : '' }}
                            ...
                            <div class="room-desc">
                                @if ($roomSetting->room_category_status == 1)
                                    <div class="room-cat">
                                        <a class="p-0 d-block"
                                            href="{{ route('front.user.rooms', ['username' => getParam(), 'category' => $latestRoom->roomCategory->id]) }}">{{ $latestRoom->roomCategory->name }}</a>
                                    </div>
                                @endif
                                <h4>
                                    <a
                                        href="{{ route('front.user.room_details', ['username' => getParam(), 'id' => $latestRoom->room_id, 'slug' => $latestRoom->slug]) }}">{{ convertUtf8($latestRoom->title) }}</a>
                                </h4>
